...
print">"
c = journalists.sort{|a, b| a.length <=> b.length}
puts c[0]

puts"Combien y-a-t'il de handle contenant 5 caractères ?" #le @ ne compte pas pour un caractère
print">"
d = journalists.select {|handle| handle.length == 6}
puts d.length

puts "Combien commencent par une majuscule ?" #(première lettre juste après le @)
first_cap_word = journalists.select { |handle| handle[1].match?(/[A-Z]/) } 
puts first_cap_word.length
...
result = journalists.sort {|min, max| min.length <=> max.length}
puts result
...
result = []
a = journalists.length
b = 1
c = []
while b != 0 do
    for i in 1..a do
      result =  journalists.select { |handle| handle.size == i }
      # puts  result.length
      b = result.length
      c[i] = result.length
    end
...
filtered_c = c.reject { |y| y == 0 }
size_filtered_c = filtered_c.length
    for i in 1..size_filtered_c-1 do
      puts "Nombre de handle avec #{i} caractère(s): #{filtered_c[i]}"
    end
